...
| 在 Part 6 已生成最终稿与审计后，按 `writing-policy/rules/scut_course_paper_format.md` 调用确定性脚本生成格式化 docx，并复制一份到桌面，文件名使用论文题目 |
...
- Part 6 docx export 只能由确定性脚本 `runtime/agents/part6_docx_exporter.py` 生成；LLM agent 不得直接生成或改写 `.docx`。
- 桌面 docx 副本必须命名为论文题目：`~/Desktop/{论文题目}.docx`。若同名文件已存在，实现应先备份旧文件，再写入同名新文件。
...
- 静默修复状态损坏
- 未经明确用户批准修改 `docs/01_build_target.md` 或 `docs/02_architecture.md` 的内容
